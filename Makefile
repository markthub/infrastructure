...
.PHONY: config
config:
	@if [ -f "terraform/kubeconfig_${CLUSTER_ID}" ]; then \
...
terraform_init: config
	...
	cd terraform && terraform workspace new ${CLUSTER_ID} 2> /dev/null || true
	cd terraform && terraform workspace select ${CLUSTER_ID}
